...
import { ShieldAlert, X, Send, FileText, CheckCircle2 } from 'lucide-react';
...
export default function EmergencyResponseModal({ isOpen, onClose, alert, onTriggerToast }) {
  ...
  const handleNotifyResponseTeam = () => {
    ...
    if (onTriggerToast) {
      onTriggerToast({
        ...
        message: `Dispatched control room alert log #${logId} to Zone E Safety Officers.`
      });
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#0D1117]/90 backdrop-blur-md z-50 flex items-center justify-center p-[16px]">
      <div className="zg-modal-panel max-w-2xl w-full relative border-[#F85149]/60">
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-[8px] mb-[16px] border-b border-[#21262D] pb-[16px]">
          ...
          <h3 className="text-[16px] font-bold text-[#F85149] uppercase tracking-[0.02em] font-mono-tech">
            AUTOMATED EMERGENCY RESPONSE ORCHESTRATOR
          </h3>
        </div>

        {/* Preliminary Incident Report Content */}
        <div className="space-y-[16px] text-[12px] font-mono-tech">
          ...
          <div className="flex items-center justify-between pt-[8px]">
            ...
            <button
              ...
              disabled={isNotified}
              className="btn-primary bg-[#F85149] border-[#F85149] text-[#0D1117] hover:bg-[#F85149]/90 disabled:opacity-50"
            >
              <Send className="w-[14px] h-[14px]" strokeWidth={1.5} />
              {isNotified ? 'Emergency Response Team Notified' : 'Notify Emergency Response Team'}
            </button>
          </div>
        </div>
      </div>
